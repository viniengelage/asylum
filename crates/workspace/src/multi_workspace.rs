...
impl MultiWorkspace {
    ...
    pub fn register_devices_sidebar<T: Sidebar>(
        ...
        if let Some(sidebar) = &self.devices_sidebar {
            sidebar.attach_to_workspace(&active_workspace, window, cx);
        }
    }
    ...
    pub fn activate(
        ...
        // The devices sidebar is shared across the workspaces in this window, so it only
        // reattaches once `workspace()` already resolves to the newly-activated one.
        if let Some(sidebar) = &self.devices_sidebar {
            sidebar.attach_to_workspace(&workspace, window, cx);
        }
