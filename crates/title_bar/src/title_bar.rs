...
impl Render for TitleBar {
    fn render(&mut self, window: &mut Window, cx: &mut Context<Self>) -> impl IntoElement {
        ...
        let project_name = self.effective_active_worktree(cx).and_then(|worktree| {
            worktree
                .read(cx)
                .root_name()
                .file_name()
        ...
        children.push(
            h_flex()
                ...
                .gap_0p5()
                .map(|title_bar| {
                    let mut render_project_name = title_bar_settings.show_project_items;
                    title_bar
                        .when_some(
                            self.application_menu.clone().filter(|_| !show_menus),
                            |title_bar, menu| {
                                render_project_name &= !menu
                        ...
                        .when(render_project_name, |title_bar| {
                            title_bar.child(self.render_project_name(project_name, window, cx))
                        })
                })
